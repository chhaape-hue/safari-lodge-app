...
import { Plus, Search, RefreshCw, CalendarDays, Trash2, CheckCircle2, AlertCircle } from "lucide-react"
import { PageLoader } from "@/components/ui/loading-spinner"

const statusConfig = {
  confirmed:   { label: "Confirmed",   variant: "info" as const },
  checked_in:  { label: "Checked In",  variant: "success" as const },
  checked_out: { label: "Checked Out", variant: "neutral" as const },
...
const sourceLabel: Record<string, string> = {
  nightsbridge: "Nightsbridge", direct: "Direct", agent: "Travel Agent",
  email: "Email", phone: "Phone", walk_in: "Walk-in",
}

export default function BookingsPage() {
  const { bookings, guests, properties, rooms, updateBooking, deleteBooking, loading, error } = useStore()
  ...
  return (
    <div>
      <Topbar title="Bookings" subtitle={`${bookings.length} bookings total`}
        actions={
          <div className="flex gap-2">
            <Button variant="secondary" size="sm"><RefreshCw className="h-3.5 w-3.5" /> NB Sync</Button>
            <Button size="sm" onClick={() => setShowForm(true)}><Plus className="h-3.5 w-3.5" /> New Booking</Button>
          </div>
        }
      />
      ...
      {!loading && <div className="p-6 space-y-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: "Checked In", value: bookings.filter(b => b.status === "checked_in").length, color: "text-[#4A7C59]" },
            { label: "Confirmed", value: bookings.filter(b => b.status === "confirmed").length, color: "text-[#6B4226]" },
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative">
            ...
            <input type="text" placeholder="Reference or guest name..." value={search}
          ...
          <div className="ml-auto flex gap-3 text-sm flex-wrap">
            <span className="text-stone-600">Revenue: <strong>{formatCurrency(totalRevenue)}</strong></span>
            <span className="text-[#4A7C59]">Paid: <strong>{formatCurrency(totalPaid)}</strong></span>
            {totalRevenue - totalPaid > 0 && <span className="text-amber-600">Outstanding: <strong>{formatCurrency(totalRevenue - totalPaid)}</strong></span>}
          </div>
        </div>
        <Card>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-100 text-xs text-stone-500 uppercase tracking-wider">
                  {["Reference","Guest","Property / Room","Check-in","Check-out","N","Amount","Source","Status",""].map(h => (
                  ...
                  return (
                    <tr key={booking.id} className="hover:bg-stone-50 transition-colors">
                      ...
                      <td className="px-5 py-3">
                        ...
                        {outstanding > 0 && <p className="text-xs text-amber-600">{formatCurrency(outstanding)} outstanding</p>}
                      </td>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex gap-1 items-center">
                          {booking.status === "confirmed" && (
                            <button onClick={() => updateBooking(booking.id, { status: "checked_in" })} title="Check In"
                          ...
                          <button onClick={() => { if (confirm("Delete this booking?")) deleteBooking(booking.id) }}
