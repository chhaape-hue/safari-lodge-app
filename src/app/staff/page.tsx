...
import { Users, Plus, Search, UserCheck, DollarSign, Building2, Trash2 } from "lucide-react"
...
const employTypeLabel: Record<EmploymentType, string> = {
  full_time: "Vollzeit", part_time: "Teilzeit", seasonal: "Saisonal", contractor: "Freelancer",
}

export default function StaffPage() {
  const { staff, properties, deleteStaff, updateStaff } = useStore()
  ...
  return (
    <div>
      <Topbar title="Personal (HR)" subtitle={`${staff.length} Mitarbeiter`}
        actions={<Button size="sm" onClick={() => setShowForm(true)}><Plus className="h-3.5 w-3.5" />Mitarbeiter hinzufügen</Button>}
      />
      <div className="p-6 space-y-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Aktive Mitarbeiter" value={activeStaff.length} icon={UserCheck} color="green" />
          <StatCard title="Monatl. Gehälter" value={formatCurrency(totalPayroll)} icon={DollarSign} color="rose" />
          <StatCard title="Abteilungen" value={new Set(staff.map(s => s.department)).size} icon={Building2} color="blue" />
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative">
            ...
            <input type="text" placeholder="Name oder Position..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          {filtered.length > 0 && (
            <span className="ml-auto text-sm text-stone-500">
              {filtered.length} Mitarbeiter · <strong>{formatCurrency(filtered.filter(s=>s.status==="active").reduce((s,m)=>s+m.salary,0))}</strong> / Monat
            </span>
          )}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-100 text-xs text-stone-500 uppercase tracking-wider">
                {["Mitarbeiter","Property","Abteilung","Position","Anstellung","Seit","Gehalt/Mo","Status",""].map(h => (
                      ...
                      <div className="flex gap-1">
                        {member.status === "active" && (
                          <button onClick={() => updateStaff(member.id, { status: "on_leave" })}
                            className="px-2 py-1 rounded text-xs text-stone-500 hover:bg-stone-100" title="Urlaub">
                            Urlaub
                          </button>
                        )}
                        <button onClick={() => { if(confirm("Mitarbeiter löschen?")) deleteStaff(member.id) }}
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-stone-400">
              ...
              <Button size="sm" className="mt-4" onClick={() => setShowForm(true)}>
                <Plus className="h-3.5 w-3.5" /> Ersten Mitarbeiter hinzufügen
              </Button>
            </div>
          )}
        </div>
